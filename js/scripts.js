document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateCart() {
        ...
        if (selectedServices.length > 0) {
            paypalContainer.innerHTML = ''; // Clear old PayPal buttons
            paypalButtonsRendered = false;  // Reset the flag
            renderPayPalButton();           // Re-render PayPal buttons
            document.getElementById("no-service-selected-note").style.visibility = "hidden";
        } else {
            paypalContainer.innerHTML = ''; // Clear button if no items
            ...
            paypal.Buttons({
                ...
                createOrder: function(data, actions) {
                    return actions.order.create({
                        purchase_units: [{
                            amount: { currency_code: "USD", value: totalWithGST.toFixed(2) }
                        }],
                        application_context: {
                ...
                onApprove: function(data, actions) {
                    return actions.order.capture().then(function(details) {
                        ...
                        localStorage.setItem('gstAmount', gst.toFixed(2)); // 18% GST
                        localStorage.setItem('totalWithGST', totalWithGST.toFixed(2)); // Final total after adding GST
